...
export default function BlogPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F0] text-[#3E2C23] font-sans">
      <Navbar />
      {/* Hero Section */}
      <section className="relative h-[60vh] flex items-center justify-center">
        ...
        >
          <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl font-bold text-white mb-4 text-center tracking-tight font-playfair-display">
            Stories That Move You
          </h1>
          <p className="text-lg sm:text-xl text-[#E6D5B8] mb-6 text-center max-w-2xl font-sans">
          ...
          {filteredPosts.slice(0, visibleCount).map((post, idx) => (
            ...
            >
              ...
              <div className="flex-1 flex flex-col p-5">
                ...
                <h2 className="font-serif text-2xl font-bold mb-2 group-hover:underline transition-colors font-playfair-display">
                  <a href={`/blog/${post.id}`} className="outline-none focus:underline" aria-label={`Read ${post.title}`}>
                    {post.title}
                  </a>
                </h2>
                <p className="text-sm text-[#5A4633] mb-4 flex-1 font-sans" aria-label="Article excerpt">{post.excerpt}</p>
